...
def der(ref_turns, sys_turns, collar=0.0, ignore_overlaps=False, uem=None):
    ...
    global_der = None
    for k in ('ALL', '*** OVERALL ***', 'OVERALL'):
        if k in file_to_der_base:
            global_der = file_to_der_base[k]
            break

    # 2) fallback: computa o DER global a partir dos TOTAIS (equivalente ao md-eval)
    if global_der is None:
        total_scored = float(scored_speaker_times.sum())
        total_error  = float(error_times.sum())
        if total_scored > 0.0:
            global_der = 100.0 * (total_error / total_scored)
        else:
            # nenhum tempo pontuado → define 0 por convenção (ou 0.0 para manter compatibilidade)
            global_der = 0.0
    # -------------------------------------------------------------------
